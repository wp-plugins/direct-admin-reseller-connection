...
function darc_login_check(){

	if (is_user_logged_in() && get_user_meta(get_current_user_id(), "darc_nh_daid", "true" ) != "") {
		return 'true';
	}else{ ?><script type="text/javascript"> 
		jQuery(document).ready(function() {
			 jQuery('#darc-nh-login-div1').load('<?php echo wp_login_url( get_permalink() ); ?> #loginform'); 
			 jQuery('#darc-nh-login-div2').load('<?php echo wp_login_url( get_permalink() ); ?> #nav'); 
		}); </script><div id="darc-nh-login-div1"></div><div id="darc-nh-login-div2"></div><?php
	}
}
...
function mb_to_gb($waarde){
		...
		$waarde = $waarde/1000; return number_format($waarde,2,',','.')."GB";
	     }
}
...
function total_percentage($hundred,$part,$return){
		$hundred = $hundred;
		$one = $hundred/100;
		if($part == $hundred){ $part = '100'; }else{ $part = $part/$one; }
		$left = 100-$part;
	if($return == 100){
		return $hundred;
	}elseif($return == "part"){
...
function darc_list_pointer($domein){
		$sock = new HTTPSocket; 
		$sock->connect(get_option('darc_nh_adresse'), get_option('darc_nh_port','2222')); 
		$sock->set_login(get_option('darc_nh_account')."|".get_user_meta(get_current_user_id(), "darc_nh_daid", "true" ), get_option('darc_nh_key')); 
		$sock->set_method('GET'); 
...
function darc_delete_pointer($domein){
		$sock = new HTTPSocket; 
		$sock->connect(get_option('darc_nh_adresse'), get_option('darc_nh_port','2222')); 
		$sock->set_login(get_option('darc_nh_account')."|".get_user_meta(get_current_user_id(), "darc_nh_daid", "true" ), get_option('darc_nh_key')); 
		$sock->set_method('GET'); 
...
function darc_update_pointer($domein,$url){
...
function darc_list_packages(){
	...
	$result = $sock->fetch_parsed_body(); 
		...
		return substr($ReqVal, 1);

}

function darc_list_packagetest($package){
		$sock = new HTTPSocket; 
		$sock->connect(get_option('darc_nh_adresse'), get_option('darc_nh_port','2222')); 
		$sock->set_login(get_option('darc_nh_account'), get_option('darc_nh_key')); 
		$sock->set_method('GET'); 
	$sock->query('/CMD_API_PACKAGES_USER', array('package' => $package));
	$result = $sock->fetch_parsed_body(); 
...
function darc_list_package($package,$value){
		$sock = new HTTPSocket; 
		$sock->connect(get_option('darc_nh_adresse'), get_option('darc_nh_port','2222')); 
		$sock->set_login(get_option('darc_nh_account'), get_option('darc_nh_key')); 
		$sock->set_method('GET'); 
