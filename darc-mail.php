...
function darc_nh_page_mail() {
 if (darc_login_check() == "true") { 
	...
	jQuery(document).ready(function($) {
	  $('div[id^=darc-dialog]').dialog({'autoOpen': false, 'dialogClass' : 'wp-dialog', width: 'auto', closeOnEnter : 'true' });
	    $('a[dialog-id^=#darc-dialog]').click(function() { event.preventDefault();
	  	var div = $(this).attr('dialog-id');
       		$(div).dialog( "open" );
	    });
	    $('input[dialog-id^=#darc-dialog]').click(function() { event.preventDefault();
	  	var div = $(this).attr('dialog-id');
       		$(div).dialog( "close" );
	    });
	 });
...
if($_POST['darc_nh_mailbox'] != "" && is_numeric($_POST['darc_nh_mailbox_size'])){
	$value = explode("@", $_POST['darc_nh_mailbox']); 
	darc_nh_update_mailbox($value[0],$value[1],$_POST['darc_nh_mailbox_size']);
}
if($_POST['darc_nh_mailbox'] != "" && $_POST['darc_nh_mailbox_delete'] != ""){
	$value = explode("@", $_POST['darc_nh_mailbox']); 
 	darc_nh_delete_mailbox($value[0],$value[1]);
}
if($_POST['darc_nh_forwarder'] != "" && $_POST['darc_nh_forwarder_delete'] != ""){
	$value = explode("@", $_POST['darc_nh_forwarder']); 
 	darc_nh_delete_forwarder($value[0],$value[1]);
}
if($_POST['darc_nh_new_mailbox_alias'] != "" AND $_POST['darc_nh_new_mailbox_domein'] != "" AND $_POST['darc_nh_new_mailbox_pass1'] != "" AND $_POST['darc_nh_new_mailbox_pass2'] != "" AND is_numeric($_POST['darc_nh_new_mailbox_size'])){
...
if($_POST['darc_nh_new_forwarder_alias'] != "" AND $_POST['darc_nh_new_forwarder_domein'] != "" AND filter_var($_POST['darc_nh_new_forwarder_email'], FILTER_VALIDATE_EMAIL)){
	...
	echo '<h2>'. __('Mailboxes', 'darc-nh').'</h2>';
	 ...
	 foreach($domeinen as $domein){ 
	   $mails = explode("aliasid", darc_list_mail($domein)); 
		...
		echo "<td> &#64; </td>";
		echo "<td>".$domein."</td>";
		echo "<td>"; if($usage[0] == 0){ _e('unlimited', 'darc-nh'); }else{ echo darc_mail_opslag($usage[0]); } echo "</td>";
...
echo '<div id="darc-dialog-edit-'.$i.'" title="'.$quota[0].'@'.$domein.'" class="wp-dialog">';

	echo '<p><form action="'.get_permalink().'" method="post"><label>'. __('Mailbox size', 'darc-nh').':</label><br>
		...
		<p align="right"><input name="darc_nh_update_size" type="submit" value="'. __('update', 'darc-nh').'"></form></p><span class="description"><i>'. __('Enter "0" for unlimited mailbox size.', 'darc-nh').'</i></span>';

echo '</div>
<div id="darc-dialog-delete-'.$i.'" title="'. __('Confirm action', 'darc-nh').'" class="wp-dialog"><form action='.get_permalink().' method="post">
...
echo '<div id="darc-dialog-new-mailbox" title="'. __('Added a new mailbox', 'darc-nh').'" class="wp-dialog" visibility="hidden">
  <p>
...
echo '<div id="darc-dialog-new-forwarder" title="'. __('Added a new forwarder', 'darc-nh').'" class="wp-dialog" visibility="hidden">
  <p>
	<table><form name="input" action='.get_permalink().' method="post">
		<tr><td colspan="2" style="padding:5px;"><label>'. __('Email address', 'darc-nh').':</label></td></tr>
